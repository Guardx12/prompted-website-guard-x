...
import { Bot, CheckCircle2, Loader2, MessageCircle, PlayCircle } from "lucide-react"
...
function TypingBubble() {
  return (
    <div className="flex items-end gap-3">
      ...
      <div className="rounded-[22px] rounded-bl-md border border-[#E8EAED] bg-white px-4 py-3 shadow-sm">
        <div className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 animate-bounce rounded-full bg-[#4285F4] [animation-delay:-0.2s]" />
          <span className="h-2.5 w-2.5 animate-bounce rounded-full bg-[#EA4335] [animation-delay:-0.1s]" />
...
function AssistantBubble({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-end gap-3">
      ...
      <div className="max-w-[86%] rounded-[22px] rounded-bl-md border border-[#E8EAED] bg-white px-4 py-3 text-[15px] leading-6 text-[#202124] shadow-sm sm:max-w-[78%]">
        {children}
      </div>
    </div>
  )
...
function UserBubble({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex justify-end">
      <div className="max-w-[86%] rounded-[22px] rounded-br-md bg-[#4285F4] px-4 py-3 text-[15px] font-medium leading-6 text-white shadow-sm sm:max-w-[78%]">
        {children}
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-[#F1F3F4]">
      <div className="mx-auto flex min-h-screen max-w-3xl flex-col">
        <div className="sticky top-0 z-20 border-b border-[#E8EAED] bg-white/95 px-4 py-3 backdrop-blur sm:px-6">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-full bg-white shadow-sm ring-1 ring-[#E8EAED]">
        ...
        <div className="sticky bottom-0 z-20 border-t border-[#E8EAED] bg-white/96 px-4 py-4 backdrop-blur sm:px-6">
          {actionArea}
        </div>
      </div>
    </div>
...
export default function ReviewCheckChatbot() {
  ...
  }, [messages, isTyping, stage, submitState])
  ...
  const chooseStanding = (value: string) => {
    ...
    setStage("reviews")
    enqueueAssistant([
      <p className="font-medium text-[#1A73E8]">Roughly how many Google reviews do you have at the moment?</p>,
    ])
  }
  ...
  const chooseReviews = (value: string) => {
    ...
    setStage("customers")
    enqueueAssistant([
      <p className="font-medium text-[#1A73E8]">Roughly how many customers do you serve in a typical month?</p>,
    ])
  }
  ...
  const chooseCustomers = (value: string) => {
    ...
    enqueueAssistant(
      [
        <>
          <p>When customers compare businesses on Google, the ones with the most recent reviews and active responses often appear more trusted and professional.</p>
          <p className="mt-3">That is usually the business that gets the enquiry first.</p>
        </>,
        <>
          <p>For example, imagine 100 potential customers searching for your service this month.</p>
          <p className="mt-3">Even if only a small portion choose a competitor because their profile looks more active, that can still mean missed enquiries without realising it.</p>
        </>,
        <>
          <p>GuardX is a fully managed review system designed to help your business look active, trusted and competitive.</p>
          <p className="mt-3">We help you generate more genuine reviews, send branded review requests, reply to reviews for you, and keep your profile looking professional.</p>
        </>,
        <>
          <p>Businesses can run it by email, SMS, or both. Some use bulk uploads, some use a simple staff form, and many just send customers weekly so we handle the rest.</p>
          <p className="mt-3">The goal is simple: keep your Google presence active enough that more customers feel confident choosing you.</p>
        </>,
        <>
          <p className="font-medium text-[#1A73E8]">You can try it free for 30 days.</p>
          <p className="mt-3">After that, most businesses invest around <span className="font-semibold text-[#1A73E8]">£25 per week</span> for the fully managed system.</p>
          <p className="mt-3">Would you like to see the review request in action, ask a question, or start your free trial?</p>
        </>,
      ],
  ...
  const showDemo = () => {
    ...
    enqueueAssistant([
      <>
        ...
        <div className="overflow-hidden rounded-2xl border border-[#E8EAED] bg-black shadow-sm">
  ...
  const actionArea = (() => {
    ...
    if (stage === "submitted") {
      return (
        <Button type="button" variant="outline" onClick={() => window.location.assign("/")} className="w-full rounded-2xl border-[#DADCE0] bg-white py-6 text-base text-[#202124] hover:bg-[#F8FAFF]">
          Back to GuardX
        </Button>
      )
    }
